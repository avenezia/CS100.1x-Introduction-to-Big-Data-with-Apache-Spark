...
uniqueWords = wordPairs.reduceByKey(lambda a,b : a + b).count()
print uniqueWords
...
def removePunctuation(text):
    ...
    """
    excludeAlphaNumericAndSpaceRe = re.compile("[^A-Za-z0-9 ]")
    return re.sub(excludeAlphaNumericAndSpaceRe, "", text).lower().strip()
...
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda row : row.split(" "))
